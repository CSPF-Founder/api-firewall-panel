...
ready(function () {
  ...
  if (updateModal) {
    updateModal.addEventListener("show.coreui.modal", (event) => {
      ...
      updateForm.querySelector('input[name="allowed_ip"]').value = entryValue;
    });
  }
  ...
  function handleUpdate(clickedBtn, restart = false) {
    ...
    const updateURL = `/endpoints/${endpointID}/allowed-ips/${entryID}`;
...
ready(function () {
  ...
  deleteBtn.addEventListener("click", function () {
    ...
    const deleteURL = `/endpoints/${endpointID}/allowed-ips/${entryID}`;
      ...
      .then(({ ok, data }) => {
        ...
        if (data.is_removed) {
          const rowToRemove = document.querySelector(
            `#allowed-ip-table tr[data-id="${entryID}"]`
          );
          if (rowToRemove) {
